...
const VirtualizedPDF = forwardRef<PdfFocusHandler, VirtualizedPDFProps>(
  ({ file, scale, setIndex, setScaleFit, setNumPages }, ref) => {
    ...
    useEffect(() => {
      ...
      async function loadFirstPage() {
        if (pdf) {
          await pdf
            .getPage(1)
            .then((page: { getViewport: (arg0: { scale: number }) => { width: number } }) => {
    ...
    return (
      <div className={`relative h-[calc(100vh-44px)] w-full border-gray-pdf bg-gray-pdf`}>
        <Document
          key={file.url}
